document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateReceipt() {
        ...
        if (selectedDeliveryOption.includes("Ultra Express Delivery")) {
            deliveryChargeAmount = 10;
        } else if( selectedDeliveryOption.includes("Express Delivery") ){
            deliveryChargeAmount = 5;
        }

        // Calculate the total cost
        const totalCostValue = calculateTotalCost(deliveryChargeAmount);

        // Display the delivery charge and total cost
        deliveryCharge.textContent = `Delivery Charge: ${formatCurrency(deliveryChargeAmount)}`;
        totalCost.textContent = `Total Cost: ${formatCurrency(totalCostValue)}`;
